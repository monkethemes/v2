<template>
	<div class="flex flex-col-reverse ft:flex-row items-center bg-alt rounded-lg w-full m-auto p-3 gap-6">
		...
		<div class="flex w-full ft:w-2/6 select-none">
			<div class="hidden ft:block relative w-0">
				<div class="absolute w-1 h-5 bg-bg"></div>
			</div>
			...
			<div class="flex w-1/2 relative">
				...
				<transition>
					<div
						...
						ref="dropdownMenu"
						class="absolute top-full w-[calc(100%+18px)] bg-alt rounded-lg text-xs text-sub pt-3 -ml-1.5 z-50"
					>
						<div
							...
							:class="[
								option === store.selectedOption ? 'select-none cursor-pointer bg-main text-alt' : '',
								'select-none cursor-pointer hover:bg-text hover:text-alt p-2'
							]"
						>
							{{ option }}
						</div>
...
const options = ['relevant', 'top all time', 'top this month', 'top this week', 'top today', 'brightest', 'darkest', 'newest', 'oldest'];
...
function handleClickOutside(event) {
	if (dropdownVisible.value && !dropdownMenu.value.contains(event.target) && !dropdownButton.value.contains(event.target)) {
		dropdownVisible.value = false;
	}
}
